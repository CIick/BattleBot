...
class WADProcessor:
    ...
    def convert_lazy_object_to_dict(self, obj: Any) -> Any:
        """Convert LazyObject/LazyList to dictionary with type information"""
        # Handle LazyList objects
        if isinstance(obj, LazyList) and self.type_list:
            try:
                ...
                for item in obj:
                    if isinstance(item, (LazyObject, LazyList)):
                        ...
                        result.append(self.convert_lazy_object_to_dict(item))
                    else:
                        result.append(item)
                return result
            except Exception as e:
        ...
        if isinstance(obj, LazyObject) and self.type_list:
            try:
                # Convert LazyObject to dictionary
                result = {}
                for key, value in obj.items(self.type_list):
                    if isinstance(value, (LazyObject, LazyList)):
                        # Recursively convert nested LazyObjects/LazyLists
                        result[key] = self.convert_lazy_object_to_dict(value)
                    elif isinstance(value, list):
                        # Handle lists that might contain LazyObjects/LazyLists
                        result[key] = [self.convert_lazy_object_to_dict(item) if isinstance(item, (LazyObject, LazyList)) else item for item in value]
                    else:
                        result[key] = value
                
                # Add type information
                type_name = self.type_list.name_for(obj.type_hash)
    ...
    def process_single_spell(self, file_path: str) -> Tuple[bool, Optional[Dict[str, Any]], Optional[Any], Optional[str]]:
        ...
        try:
            ...
            if isinstance(spell_data, LazyObject):
                spell_dict = self.convert_lazy_object_to_dict(spell_data)
            else:
...
def convert_lazy_object_to_dict(obj: Any, type_list: Optional[TypeList]) -> Any:
    """Compatibility function for existing code"""
    processor = WADProcessor()
    processor.type_list = type_list
    return processor.convert_lazy_object_to_dict(obj)